          ...
          <tbody>
            @foreach($reviews as $review)
                <tr>
                    ...
                    <td>{{$review->parking->title}}</td>
                    <td>{{$review->review}}</td>
                    <td>
                     <ul style="list-style:none" class="d-flex">
                          @for($i=1; $i<=5;$i++)
                          @if($review->rate >=$i)
                            <li style="float:left;color:#00414A;"><i class="fa fa-star"></i></li>
                          @else
                            <li style="float:left;color:#00414A;"><i class="far fa-star"></i></li>
                          @endif
                        @endfor
                     </ul>
                    </td>
                    ...
                    <td>
                        <a href="{{route('user.parkingreview.edit',$review->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                        <form method="POST" action="{{route('user.parkingreview.delete',[$review->id])}}">
                          @csrf
                          @method('delete')
